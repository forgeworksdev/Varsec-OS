...
func _init() -> void:
	...
	if self not in Core.opened_programs:
		Core.opened_programs.append(self)
...
func _on_viewport_size_changed():
	self.size.x = min(self.size.x, Core.viewport_size.x)
	self.size.y = min(self.size.y, Core.viewport_size.y - Core.panel_height)
	global_position.x = clamp(global_position.x, 0, Core.viewport_size.x - self.size.x)
	global_position.y = clamp(global_position.y, Core.panel_height, Core.viewport_size.y - self.size.y)
...
func maximize_window(target: window_vos) -> void:
	...
	new_size.y -= Core.panel_height
	var new_position: Vector2 = Vector2(0, Core.panel_height)
...
func select():
	get_parent().move_child(self, Core.opened_programs.size())
...
func resize_process() -> void:
	if is_resizing and can_resize:
		...
		if Input.is_key_pressed(KEY_SHIFT):
			...
			self.size.x = min(self.size.x, Core.viewport_size.x - global_position.x)
			self.size.y = min(self.size.y, Core.viewport_size.y - global_position.y)
		if has_max_size:
